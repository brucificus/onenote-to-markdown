...
def _create_colspec_cleaning_element_predicate_filter_pairs() -> Sequence[predicate_filter_pair]:
    def create_colspec_update_filters_for_colspec_cleanup() -> Iterable[predicate_filter_pair]:
        def elements_predicate(element: panflute) -> bool:
        ...
        def update_element_colspec(element: panflute.Table, _: panflute.Doc) -> PanfluteElementLike:

            element.colspec = ((c[0], panflute_table_colspec_default_width_specifier) for c in element.colspec)
            return element

        yield elements_predicate, update_element_colspec

    _colspec_cleaning_element_predicate_filter_pairs: Sequence[predicate_filter_pair] = (
        *create_colspec_update_filters_for_colspec_cleanup(),
    )

    return _colspec_cleaning_element_predicate_filter_pairs
