...
import onenote_export.page_run_pandoc_conversion_to_markdown
...
class OneNotePageExporter(OneNoteExportTaskBase):
    ...
    def _yield_subtasks(context: OneNotePageExportTaskContext, prerequisites: Tuple[OneNoteExportTaskBase], subtask_factory: OneNoteExportTaskFactory) -> Iterable['OneNoteExportTask']:
        ...
        task_run_pandoc_conversion_to_markdown = create_subtask(context.node, task_spec=onenote_export.page_run_pandoc_conversion_to_markdown.page_run_pandoc_conversion_to_markdown, prerequisites=(task_ensure_output_dir_exists, task_ensure_assets_dir_exists))
        yield task_run_pandoc_conversion_to_markdown

        task_pdf_patch_images_into_md = create_subtask(context.node, task_spec=onenote_export.page_pdf_patch_images_into_md.page_pdf_patch_images_into_md, prerequisites=(task_run_pandoc_conversion_to_markdown,))
